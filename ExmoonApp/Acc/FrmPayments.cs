...
namespace ExmonSystem.Form
{
    public partial class FrmPayments : DevExpress.XtraEditors.XtraForm
    {
        ...
        public string transType { get; set; } = "سند صرف";
         public string EntryType { get; set; }
        public int SelectedaccId { get; set; } = 0;
        ...
        async Task getAccountsName()
        {
            if (formType == "Payment")
            {
                ...
                var xx = await accControl.GetAllAccount(mAccountId==102?101: mAccountId);
                ...
                var xxto = Program.userCashAccountId == 0 ? await accControl.GetAllAccount(mAccountId==102?102:101) : await accControl.Getaccounts(Program.logedUser.CashAccountId);
            ...
            }  else if (formType == "D Entry")
            {
                ...
                var xx = await accControl.GetAllAccount(0);
                foreach (var item in xx)
                {
                    elistFrom.Add(new ExampleFrom() { Id = item.Id, AccountNo = item.AccountID, AccountName = item.AccountName, Phone = item.phone, TaxNumber = item.taxnumber });
                ...
                var xxto = await accControl.GetAllAccount(0);
                foreach (var item in xxto)
                {
                    elistTo.Add(new ExampleTo() { Id = item.Id, AccountNo = item.AccountID, AccountName = item.AccountName, Phone = item.phone, TaxNumber = item.taxnumber });
        ...
        private async void FrmPayments_Load(object sender, EventArgs e)
        {
...

            {
                ...
                if (Properties.Settings.Default.Language == "ar")
                {
                    LblHeader.Text = "سند قيد يومية";

                    lblFrom.Text = "الحساب المدين";
                    lblTo.Text = "الحساب الدائن";
                }
                else
                {
                    LblHeader.Text = "Daily Entry";

                    lblFrom.Text = "Debit Account";
                    lblTo.Text = "Credit Account";
            ...
            if (formType == "Payment") //سندصرف
            {
                ...
                if (PaymentType== "صرف عمولة توصيل"||PaymentType== "صرف عمولة تسليم")
                {
                    lblFrom.Visible = false;
                    cmbFrom.Visible = false;
                    TxtFromBalance.Visible = false;
                ...
                else
                {
                    ...
                    if (SelectedaccId > 0)
                    {
                        cmbFrom.EditValue = SelectedaccId;
                        TxtAmount.Focus();
                    }
                }
        ...
        int MvoucherId = 0;/// <summary>
        /// the Main Voucher ID if this is tax Entry
        /// </summary>
        /// <param name="MentryId"></param>
        /// <returns></returns>
        //async Task<bool> addtax(int MentryId )
        //{
        ...
        //        await accControl.DeleteTransAccTax(MentryId, formType);
        //        entryId = await accControl.GetDefaultEntryId(formType);
      ...
      async  Task getFromBalance()
       ...
       async Task amountChanged()
        ...
        {
            XtraReport rd = XtraReport.FromFile(System.Windows.Forms.Application.StartupPath + @"\rpt\" + "RptVoucher.repx", true);
