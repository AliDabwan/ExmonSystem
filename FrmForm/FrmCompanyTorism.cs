...
{
    public partial class FrmCompanyTorism : DevExpress.XtraEditors.XtraForm
    {
        ItemGoverControl accControl = new ItemGoverControl();

        public FrmCompanyTorism()
        {
            InitializeComponent();
        }
        ...
        async Task GetLoad()
        {
            ...
            companyTorismBindingSource.DataSource = await accControl.GetAllCompanyTorisms();//db.itemCategories.ToList();

            TxtName.Enabled = true;

            companyTorismBindingSource.Add(new CompanyTorism());
            companyTorismBindingSource.MoveLast();
            TxtName.Focus();
        ...
        private void BtnAdd_Click(object sender, EventArgs e)
        {
            ...
            companyTorismBindingSource.MoveLast();
            CompanyTorism obj = companyTorismBindingSource.Current as CompanyTorism;
            if (obj != null)
            {
                if (obj.Id == 0)
                {
        ...
        private async void  BtnSave_Click(object sender, EventArgs e)
        {
            ...
            using (MyDbContext db = new MyDbContext())
            {
                ...
                if (obj != null)
                {
                    if (db.Entry<CompanyTorism>(obj).State == EntityState.Detached)
                        db.Set<CompanyTorism>().Attach(obj);
                    if (obj.Id == 0)

                        db.Entry<CompanyTorism>(obj).State = EntityState.Added;
                    else
                        db.Entry<CompanyTorism>(obj).State = EntityState.Modified;
                  var rslt=await  db.SaveChangesAsync();
                    ...
                    if (rslt>0)
                    {
                        ...
                        companyTorismBindingSource.ResetBindings(false);
            ...
            if (DevExpress.XtraBars.Docking2010.Customization.FlyoutDialog.Show(this, action, properties, predicate) == System.Windows.Forms.DialogResult.Yes)
            {
                using (MyDbContext db = new MyDbContext())
                {
                    ...
                    if (obj != null)
                    {
                        if (db.Entry<CompanyTorism>(obj).State == EntityState.Detached)
                            db.Set<CompanyTorism>().Attach(obj);
                        db.Entry<CompanyTorism>(obj).State = EntityState.Deleted;
                        db.SaveChanges();
                        ...
                        companyTorismBindingSource.RemoveCurrent();
                        TxtName.Enabled = false;
                        BtnAdd_Click(sender, e);
